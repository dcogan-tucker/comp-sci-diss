...
public class SphereSlopeDemo extends Application
{
	...
	protected void initScene(Camera camera)
	{
		...
		MovableCollidableBall sphere = MovableCollidableBall.create(
				...
				new Vector3f(0, 0, 0), 
				0.025f, 1);
		slope.getComponent(Collidable.class).restitution = 0.4f;
		sphere.getComponent(Collidable.class).restitution = 0.8f;
		slope.getComponent(Collidable.class).friction = 0.2f;
		sphere.getComponent(Collidable.class).friction = 0.2f;
	}
